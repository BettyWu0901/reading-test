...
def call_ai_grade_qa(question, student_answer, story_text):
    ...
    # === 新版優化 Prompt：強調獨特回饋與嚴格格式 ===
    prompt = f"""
    請扮演《神奇柑仔店》的紅子老闆娘，為國小學生的問答題提供**獨特且針對性的回饋**。
    ...
    【**回饋與評分原則**】：
    1. **獨特回饋**：請根據學生的「【學生回答】」內容，給予針對性的稱讚或引導，避免制式化的評語。
    2. **錯誤處理**：如果「【學生回答】」內容為空白、無意義（例如：「我忘了」、「不知道」），分數必須為 **0 分**，並在評語中指出「請認真作答」。
    3. **格式嚴格**：**你的回答必須且只能是**「分數|評語」，評語必須是完整的繁體中文句子。

    範例輸出格式（**嚴格遵守，不可多加其他文字**）：
    15|你的回答「看客人的特質」很正確，但缺少了兩個故事例子佐證，請再想想。
    """
    # ==========================
    ...
    try:
        ...
        # 容錯處理：若 AI 輸出的獨特回饋格式錯誤，轉為引導學生檢查內容結構
        # 修正點 3: 避免使用「格式錯誤」，改為引導內容結構
        return 0, "紅子老師無法從你的回答中，清楚判斷「道理」和「兩個故事例子」。請確保你的回答結構完整，並重新作答喔！ (本次不計分)"
    except Exception:
        # 如果連線失敗，回傳 0 分
        return 0, "錢天堂暫時連線不順暢。請稍候，等紅子老師準備好再試一次！ (本次不計分)"
...
def call_ai_final_comment(total, level, story_text):
    ...
    prompt = f"""
    你是一位簡潔且專業的閱讀老師。學生在本次《神奇柑仔店》閱讀測驗中獲得了 {total} 分 (滿分100)。

    請嚴格使用「繁體中文」撰寫一份總結報告。你的輸出必須直接是報告內容，前面不能有任何多餘的解釋或開頭語。

    【報告結構】：
    請嚴格模仿以下範例結構，並在每個標題上使用 **粗體標籤**：
    1. **總結判斷** (1句，根據分數判斷通過/優秀/需加強)。
    2. **學習優勢** (1-2句，指出他們在理解故事主題、寓意或細節上的優點)。
    3. **閱讀技巧建議** (1句，給出一個具體可執行的、**針對長篇閱讀或全書理解**的策略指導)。

    【格式要求】：
    * 全程使用繁體中文。
    * 必須使用 Markdown 格式，並且 **嚴格遵守** 以下範例的換行與粗體結構。

    範例格式（**請參考這些建議，並輪流使用更合適的建議**）：
    **總結：** 本次測驗未達合格標準，請再接再厲。
    **優勢：** 我們看見你在這次測驗中展現了堅持作答的態度，這是一份寶貴的學習潛力。
    **技巧：** **建議下次在閱讀全書時，可以隨手記錄每篇故事的「主角、零食與結局」，有助於結構化記憶。**
    """
    try:
        model = genai.GenerativeModel('gemini-2.5-flash')
